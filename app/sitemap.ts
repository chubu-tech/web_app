...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  /*
    The merge put the marketing site and the product on one domain, so one sitemap has to
    carry both. Three of these came from `landing_page/app/sitemap.ts`, which is now gone.

    **`/` changed meaning.** It used to be Discover, listed `daily` because a salon list
    turns over. It is the marketing homepage now — `monthly` is honest for it — and Discover
    moved to `/discover`, which keeps the daily cadence and the second-highest priority: it
    is the page that leads to every salon page below.
  */
  const fixed: MetadataRoute.Sitemap = [
    ...
    { url: absoluteUrl("/waitlist"), changeFrequency: "monthly", priority: 0.3 },
