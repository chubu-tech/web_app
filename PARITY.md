...
| A3 | ~~**"Book again"**~~ — **the row was removed on 2026-08-19 by product decision, so this is now a deliberate divergence from the app rather than a parity item.** The resolver underneath it is untouched: `lib/rebook.ts` still resolves a past booking against the salon's **current** menu and still carries its 22 tests, so putting the row back is a component and a prop rather than a re-port. What was deleted is `BookAgainRow`, `startRebook`, Discover's `pastBookings` prop and the `fetchMyBookings` read behind it — one fewer round trip on every signed-in Discover load. | `lib/rebook.ts` (22 tests) — **no caller in this repo** |
