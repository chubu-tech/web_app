...
/*
  Where a stripped or missing `?next=` lands somebody.

  `/discover` since the merge, not `/`: this constant is only ever reached from an auth
  flow, and `/` is the marketing homepage now. Sending a customer who just signed in to a
  page selling them the product is worse than sending them nowhere.
*/
export const DEFAULT_NEXT = "/discover";
